...
void loop() {
  ...
  radio.receive((char*)(data));

  last_rssi = radio.get_rssi_last();
  
  bmp.measureTemperatureAndPressure(T, P); // get presure on the ground
  ...
  altitiude = 44330 * (1.0 - pow(data[1] / P, 0.1903));

  log_to_sd (last_rssi, data[0], data[1], P, data[2], altitiude);

  print_to_lcd(last_rssi, data[0], altitiude);
}
